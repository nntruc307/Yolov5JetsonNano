...
def sign_catch(list_det, thresh): # List of detected signs, thresh is the minimum number of detected signs have presented, to avoid noise and wrong detections
    ...
    priority_list = ['SL 100','SL 120','SL 80', 
                    'SL 70','SL 60','SL 50',
...
model = YoloV5TRT(library = "yolov5/build/libmyplugins.so", engine = "TSD_320.engine", conf = 0.7)
...
window_width = 960 # Display window with specific Width
window_height = 540 # Display window with specific height
...
while True:
    ...
    cv2.putText(frame, max_spd_text, (frame_width - 400, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
    print(max_spd_text)
    min_spd_text = "Min speed limited: " + str(min_speed_limited[-1])
    cv2.putText(frame, min_spd_text, (frame_width - 400, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
    print(min_spd_text)
    ...
    veh_spd_disp = "Current Vehicle Speed: {:.1f}".format(current_veh_spd)
    print(veh_spd_disp)
    # Put current vehicle speed to display window
    cv2.putText(frame, veh_spd_disp, (frame_width - 400, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
